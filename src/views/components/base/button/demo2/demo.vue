<template>
  <um-box page>
    <el-row>
      ...
      <br />
      <um-button-delete :options="{ type: 'danger' }" :action="remove" id="1" @success="onSuccess" />
    </el-row>
    ...
    <el-row style="height:400px">
      ...
      <um-list v-bind="list">
        <template v-slot:querybar-buttons="{ selection }">
          <um-button-delete-batch :action="remove" :selection="selection" show-property="name" @success="onSuccess" />
        </template>
      </um-list>
    </el-row>
    <el-row style="height:400px">
      ...
      <p>
        <br />
        <um-button-copy copy="复制的内容" /></p
    ></el-row>

    <!--在线编辑预览不支持闭合的方式，请使用下面的代码方式-->
    <!-- <um-button>默认按钮</um-button> -->
  </um-box>
</template>
<script>
export default {
