    ...
    modules.forEach(m => {
      ...
      if (m.components) {
        m.components.forEach(c => {
          ...
          // 判断是否是登录组件
          if (c.name.startsWith('um-login-')) {
            loginComponents.push(c.name.replace('um-login-', ''))
          }
          // 判断是否是登录组件
          if (c.name.startsWith('um-toolbar-')) {
            customToolbars.push(c.name.replace('um-toolbar-', ''))
          }
        })
      }
    })
