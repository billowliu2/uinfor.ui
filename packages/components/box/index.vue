<template>
  <section :class="class_" :style="style_" v-loading="loading" :element-loading-text="loadingText" :element-loading-background="loadingBackground" :element-loading-spinner="loadingSpinner">
    <!--头部-->
    <header v-if="header" class="um-box-header">
      <slot name="header">
        <div v-if="icon" class="um-box-header-icon">
          <um-icon v-if="icon" :name="icon" />
        </div>
        <!--标题-->
        <div class="um-box-header-title">
          <slot name="title">{{ title }}</slot>
        </div>
        <!--工具栏之前-->
        <div class="um-box-header-toobar-before">
          <slot name="toolbar-before" />
        </div>
        <!--工具栏-->
        <div ref="toolbar" class="um-box-header-toolbar">
          <!--工具栏插槽-->
          <slot name="toolbar" />
          <!--刷新按钮-->
          <um-button v-if="refresh" icon="refresh" @click="onRefresh" />
          <!--折叠按钮-->
          <um-button v-if="collapse" :icon="collapse_ ? 'angle-down' : 'angle-up'" @click="onCollapse" />
          <!--全屏按钮-->
          <um-button v-if="fullscreen" :icon="fullscreen_ ? 'min' : 'max'" @click="onFullscreen" />
        </div>
      </slot>
    </header>
    <el-collapse-transition>
      <section class="um-box-dialog" v-show="!collapse_">
        <section class="um-box-content">
          <section class="um-box-wrapper" v-if="hasScrollbar">
            <um-scrollbar ref="scrollbar" :horizontal="horizontal">
              <slot />
            </um-scrollbar>
          </section>
          <slot v-else />
        </section>
        <footer v-if="footer" :class="['um-box-footer', footerAlign]">
