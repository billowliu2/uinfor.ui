<template>
  ...
  <el-popover ref="popover" :popper-class="popperClass" v-model="visible" :width="width">
    <um-box page header no-scrollbar :title="title_" :icon="icon" footer :loading="showLoading">
      <template v-slot:toolbar>
        <!--全部折叠按钮-->
        <el-tooltip v-if="collapseAll" effect="dark" :content="collapsed ? '展开全部' : '折叠全部'" placement="top">
          <um-button :icon="collapsed ? 'unfold' : 'fold'" @click="onCollapseAll" />
        </el-tooltip>
        <!--刷新按钮-->
        <um-button v-if="refresh" icon="refresh" @click="refreshTree" />
      </template>
      <section class="um-tree-select-content">
        <!--搜索框-->
        <div class="um-tree-select-search-wrapper">
          <el-input :size="fontSize" v-if="filterable" class="um-tree-select-search" v-model="keyword" placeholder="请输入内容" prefix-icon="el-icon-search"> </el-input>
        </div>
        <!--树结构-->
        <div class="um-tree-select-tree-wrapper">
          <div class="um-tree-select-tree-main">
            <um-scrollbar horizontal>
        ...
        <div style="float:left">
          <span style="float:left;font-size:12px;">
            ...
            <span v-if="multiple && multipleLimit > 0"
              >(<label class="um-size-13 um-text-danger um-p-3">{{ selection.length }}/{{ multipleLimit }}</label
              >)</span
            >
          </span>
        </div>
        <el-button type="text" @click="save">确定</el-button>
        <el-button class="um-text-warning" type="text" @click="reset">重置</el-button>
        <el-button class="um-text-info" type="text" @click="visible = false">取消</el-button>
      </template>
    </um-box>
    <!--展示-->
    <el-input
...
export default {
  ...
  computed: {
    popperClass() {
      return `um-tree-select-popover ${this.filterable ? 'filterable' : ''}`
    },
    selection_() {
