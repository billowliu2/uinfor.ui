<template>
  <component v-bind:is="`um-${options.showMode || 'drawer'}`" header footer draggable :padding="10" :title="title" :icon="icon" :width="width" :height="height" :visible.sync="visible_">
    <el-form ref="form" :model="model" :rules="rules" :size="fontSize" label-width="120px">
        ...
        <el-table ref="table" :data="columns" border stripe size="mini" max-height="400" @selection-change="onSelectionChange">
          ...
          <el-table-column prop="width">
            <template slot="header"
              >宽度 <el-tooltip effect="dark" content="0表示自适应" placement="top"> <i class="el-icon-info um-size-15 um-text-warning"> </i></el-tooltip
            ></template>
            <template v-slot="{ row }">
          ...
          <el-table-column prop="format">
            <template slot="header"
              >格式化 <el-tooltip effect="dark" content="如日期：yyyy-MM-dd" placement="top"> <i class="el-icon-info um-size-15 um-text-warning"> </i></el-tooltip
            ></template>
            <template v-slot="{ row }">
      ...
      <el-collapse>
        <el-collapse-item>
          <template slot="title"
            ><span class="um-p-l-10">
              <i class="el-icon-s-tools um-size-16" style="position: relative;top: 2px;"></i>
              高级选项</span
            >
          </template>
          <el-row>
    ...
    <template v-slot:footer>
      <um-button type="success" text="导出" @click="onExport" />
      <um-button type="info" text="取消" @click="hide" />
    </template>
  </component>
</template>
<script>
