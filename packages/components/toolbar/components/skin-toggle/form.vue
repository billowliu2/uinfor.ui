<template>
  <um-drawer class="um-skin-toggle" v-bind="deawer" :visible.sync="visible_" v-on="on">
      ...
      <div v-for="item in list" :key="item.code" :class="['skin-list-item', model.name === item.code ? 'active' : '']" @click="onSkinChange(item)">
        ...
        <div class="text">
          ...
          <el-popover placement="left" :title="item.name" width="700" trigger="click">
            ...
            <um-button slot="reference" type="text" text="查看大图" />
          </el-popover>
        </div>
      </div>
    </section>
    ...
    <template v-slot:footer>
      <um-button type="success" text="保存" @click="onSave" />
    </template>
  </um-drawer>
</template>
<script>
