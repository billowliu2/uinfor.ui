<template>
  ...
  >
    ...
    <template v-slot:title>
      <section v-if="icon" class="um-dialog-icon">
        <um-icon :name="icon" :style="{ color: iconColor || '' }" />
      </section>
      <section ref="title" class="um-dialog-title">
        <slot name="title">{{ title }}</slot>
      </section>
      <section class="um-dialog-toolbar">
        <!--工具栏插槽-->
        <slot name="toolbar" />
        <!--全屏按钮-->
        <um-button v-if="fullscreen" :icon="hasFullscreen ? 'fullscreen-c' : 'fullscreen-o'" @click="triggerFullscreen" />
        <!--关闭按钮-->
    ...
    <!--Body-->
    <section ref="dialogBody" class="um-dialog-body" v-loading="loading" :element-loading-text="loadingText" :element-loading-background="loadingBackground" :element-loading-spinner="loadingSpinner">
      <section class="um-dialog-content">
        <section class="um-dialog-main">
          <slot v-if="noScrollbar" />
          <um-scrollbar v-else ref="scrollbar" class="um-dialog-body-scrollbar">
            <slot />
          </um-scrollbar>
        </section>
      </section>
      <section v-if="footer" class="um-dialog-footer">
        <div class="um-form-footer-left">
          <slot name="footer-left" />
        </div>
        <div class="um-form-footer-right">
          <slot name="footer" />
          <!--底部关闭按钮-->
          <um-button type="info" v-if="footerCloseButton" text="关闭" @click="close" />
        </div>
      </section>
    </section>
  </el-dialog>
...
export default {
  ...
  computed: {
    ...
    class_() {
      return ['um-dialog', this.draggable_ ? 'draggable' : '', this.header ? '' : 'no-header']
    },
    width_() {
  ...
  methods: {
    ...
    resize() {
      ...
      if (!this.hasInit) {
        ...
        // 设置内边距
        if (this.padding) {
          this.dialogEl.querySelector(this.noScrollbar ? '.um-dialog-main' : '.el-scrollbar__view').style.padding = this.padding + 'px'
        }
        this.hasInit = true
      }
    ...
    getDialogHeight() {
      ...
      const footerEl = this.dialogEl.querySelector('.um-dialog-footer')
