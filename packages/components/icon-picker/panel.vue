<template>
  <um-dialog v-bind="dialog" v-on="dialogOn" :visible.sync="visible_">
    <template v-slot:toolbar>
      <el-input class="um-icon-picker-panel-filter" v-model="filter" placeholder="请输入英文或中文名称" clearable></el-input>
    </template>
    <section class="um-icon-picker-panel">
      <el-tooltip v-for="icon in filterList" :key="icon" effect="dark" :open-delay="800" :content="icon" placement="top">
        <div class="um-icon-picker-panel-item" @click="onSelect(icon)">
          <um-icon :name="icon" />
        </div>
      </el-tooltip>
    </section>
