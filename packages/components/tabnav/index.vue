<template>
  <section :class="['um-tabnav', fontSize]">
    <slot name="before" />
    <div class="um-tabnav-tabs">
      <el-tabs :value="current.path" type="card" :closable="true" @tab-click="onTabClick" @tab-remove="onTabRemove" @contextmenu.prevent.native="showContextMenu">
        <el-tab-pane v-if="config.showHome" :name="defaultPage">
          <span slot="label"> <um-icon v-if="config.showIcon" name="home" /> 首页</span>
        </el-tab-pane>
        <el-tab-pane v-for="(item, i) in opened" :key="item.path" :name="item.path"
          ><span slot="label" :index="i"> <um-icon v-if="config.showIcon && item.icon" :name="item.icon" />{{ item.tabName }}</span>
        </el-tab-pane>
      </el-tabs>
    </div>
    <div class="um-tabnav-control">
      <el-dropdown @command="cmd => handleCommand(cmd)">
        <span class="um-tabnav-control-btn">
          <um-icon name="angle-double-down" />
        </span>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="Left"> <um-icon name="arrow-left" class="um-tabnav-control-icon" />关闭左侧 </el-dropdown-item>
          <el-dropdown-item command="Right"> <um-icon name="arrow-right" class="um-tabnav-control-icon" />关闭右侧 </el-dropdown-item>
          <el-dropdown-item command="Other"> <um-icon name="other" class="um-tabnav-control-icon" />关闭其他 </el-dropdown-item>
          <el-dropdown-item command="All"> <um-icon name="app" class="um-tabnav-control-icon" />全部关闭 </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <div ref="contextmenu" class="um-tabnav-contextmenu" v-show="contextmenu.visible" :style="{ top: contextmenu.top, left: contextmenu.left }">
      <ul>
        <li @click.stop="onRefresh"><um-icon name="refresh" />重新载入</li>
        <li @click.stop="onClose('')"><um-icon name="close" />关闭</li>
        <li @click.stop="onClose('Left')"><um-icon name="arrow-left" />关闭左侧</li>
        <li @click.stop="onClose('Right')"><um-icon name="arrow-right" />关闭右侧</li>
        <li @click.stop="onClose('Other')"><um-icon name="other" />关闭其它</li>
        <li @click.stop="onClose('All')"><um-icon name="app" />关闭全部</li>
      </ul>
    </div>
  </section>
</template>
